...
class wysiwyg
{
  ...
  public function parseBlock($block, $sanitize = true, $strip = true, $paragraph = true)
  {
    ...
    ***/
    ...
    if (!$strip) {
      ...
      if ($raise_error) {
          return array('status' => true,'html' => $parsed,'error_log' => $error_log,'new_edit' => deparseBlock($parsed));
      }

      return $parsed;
  }

    public function deparseBlock($block)
    {
        $parsed = $block;
    // Un-replace fixes
    $parsed = stripslashes($parsed);
